...
class CreateOrderService {
  public async execute({customer_id, products}: IRequest):Promise<Order>{
    ...
    const quantityAvailable = products.filter(product => existsProducts.filter(
        prod => prod.id === product.id)[0].quantity < product.quantity);

    if(quantityAvailable.length){
        throw new AppError(
            `Could not find any product ${quantityAvailable[0].quantity} 
    ...
    const updatedProductQuantity = order_products.map(product => ({
        ...
        quantity: existsProducts.filter(
            prod => prod.id === product.product_id)[0].quantity - product.quantity
    }))
